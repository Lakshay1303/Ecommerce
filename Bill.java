...
import callFromConsole.Product;
...
public class Bill extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			Connection connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/callconsole", "root",
					"mysqlroot");
			...
			ResultSet set1 = statement1.executeQuery("select * from customer where `username`='" + u + "' ");
			while (set1.next()) {
			...
			while (set.next()) {
				...
				for (Product p : list) {
					if (id == p.getId()) {
						...
						statement.executeUpdate("UPDATE `callconsole`.`customer` SET `bill`='" + bill
								+ "' WHERE(`username`='" + u + "')");
					}
